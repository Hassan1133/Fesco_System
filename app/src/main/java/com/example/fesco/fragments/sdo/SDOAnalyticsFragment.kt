...
import com.example.fesco.activities.sdo.SDOLSAnalyticsActivity
import com.example.fesco.adapters.LSDropDownAdp
import com.example.fesco.databinding.FragmentSDOAnalyticsBinding
...
class SDOAnalyticsFragment : Fragment() {

    private lateinit var binding: FragmentSDOAnalyticsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentSDOAnalyticsBinding.inflate(inflater, container, false)
        init()
    ...
    private fun getLSArrayFromSharedPreferences() {
        val lsArrayString =
            requireActivity().getSharedPreferences("sdoData", AppCompatActivity.MODE_PRIVATE)
                ?.getString(
                    "ls", null
                ) // Get LM data as JSON string
        val lsArray = lsArrayString?.let {
            Gson().fromJson(
        ...
        lsArray?.let { getLSDataFromDb(it) } // Get LM data from Firestore using array
    }

    private fun getLSDataFromDb(lsArray: Array<String>) {
        lifecycleScope.launch { // Use coroutine for asynchronous operations
            lsList = lsArray.mapNotNull { lsID -> // Map LM IDs to LM objects
                try {
    ...
    private fun getLMNameFromLmList(list: List<LSModel>) {
        val adapter = LSDropDownAdp( // Create adapter for LM dropdown
            requireActivity(), list
        )

        binding.selectLS.setAdapter(adapter) // Set adapter to dropdown

        binding.selectLS.setOnItemClickListener { _, _, position, _ ->
            val lsModel = adapter.getItem(position)
            lsModel?.let {
                binding.selectLS.setText(it.name)
                lsId = it.id
            }
        }
    }
    ...
    private fun isValid(): Boolean {
        ...
        if (lsId.isEmpty()) {
            binding.selectLS.error = "select LM please"
            valid = false
        }
